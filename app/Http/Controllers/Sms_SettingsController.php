...
class Sms_SettingsController extends Controller
{
    ...
    public function get_sms_config(Request $request)
    {
		...
		if ($user_auth->can('sms_settings')){
            $eskiz['ESKIZ_TOKEN'] = env('ESKIZ_TOKEN');
            $eskiz['ESKIZ_EMAIL'] = env('ESKIZ_EMAIL');
            $eskiz['ESKIZ_PASSWORD'] = env('ESKIZ_PASSWORD');
            ...
            return view('settings.sms_settings', compact('eskiz','twilio','nexmo','infobip','default_sms_gateway'));
    ...
    public function update_eskiz_config(Request $request)
    {
        $user_auth = auth()->user();
		if ($user_auth->can('sms_settings')){

            $this->setEnvironmentValue([
                'ESKIZ_TOKEN' => $request['ESKIZ_TOKEN'] !== null?'"' . $request['ESKIZ_TOKEN'] . '"':'"' . env('ESKIZ_TOKEN') . '"',
                'ESKIZ_EMAIL' => $request['ESKIZ_EMAIL'] !== null?'"' . $request['ESKIZ_EMAIL'] . '"':'"' . env('ESKIZ_EMAIL') . '"',
                'ESKIZ_PASSWORD' => $request['ESKIZ_PASSWORD'] !== null?'"' . $request['ESKIZ_PASSWORD'] . '"':'"' . env('ESKIZ_PASSWORD') . '"',
            ]);
